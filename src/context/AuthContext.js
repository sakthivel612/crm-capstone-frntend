...
export const AuthContextProvider = ({ children }) => {
  ...
  const checkUserLoggedIn = async () => {
    try {
      const res = await fetch(
        `https://contact-crm-backend-08fb.onrender.com/api/me`,
        {
          method: "GET",
          headers: {
  ...
  const loginUser = async (userData) => {
    try {
      const res = await fetch(
        `https://contact-crm-backend-08fb.onrender.com/api/login`,
        {
          method: "POST",
          headers: {
  ...
  const registerUser = async (userData) => {
    try {
      const res = await fetch(
        `https://contact-crm-backend-08fb.onrender.com/api/register`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
